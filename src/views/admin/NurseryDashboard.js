...
const StyledDashboard = styled.div`max-width: 1200px;`;
...
const StyledSection = styled.section`margin-bottom: 4rem;`;
...
const StyledRejectionTitle = styled.h1`color: #721c24; font-size: 2.5rem; margin-bottom: 1rem;`;
const StyledRejectionText = styled.p`color: #721c24; font-size: 1.2rem; margin-bottom: 2rem; max-width: 600px;`;
const StyledRejectionButton = styled(Button)`background: #721c24; &:hover { background: #5a1620; }`;
...
const NurseryDashboard = () => {
    ...
    const [editForm, setEditForm] = useState({ plantTitle: '', plantPrice: '', plantDescription: '', plantImage: '' });
    ...
    if (!myNursery && nurseries.length > 0) {
        return <AdminTemplate><StyledDashboard><Heading>Loading Nursery Data...</Heading></StyledDashboard></AdminTemplate>;
    }
        ...
        return (
            <AdminTemplate>
                <StyledRejectionContainer>
                    ...
                    <StyledRejectionText>
                        Unfortunately, your nursery application has been rejected.
                        Please contact support for more information.
                    </StyledRejectionText>
                    <StyledRejectionButton onClick={() => { window.location.href = '/'; }}>
                        Return Home
    ...
    return (
        <AdminTemplate>
            <StyledDashboard>
                {/* Edit Modal */}
                {editingPlant && (
                    <Overlay onClick={(e) => { if (e.target === e.currentTarget) setEditingPlant(null); }}>
                        <Modal>
                            ...
                            <form onSubmit={handleEditSave}>
                                ...
                                <FormGroup>
                                    ...
                                    <StyledInput id="ed-title" name="plantTitle" value={editForm.plantTitle} onChange={handleEditChange} placeholder="e.g. Monstera Deliciosa" required />
                                </FormGroup>
                                <Grid2 style={{ marginTop: '0.8rem' }}>
                                    <FormGroup>
                                        <Label htmlFor="ed-price">Price ($) *</Label>
                                        <StyledInput id="ed-price" name="plantPrice" type="number" min="0" step="0.01" value={editForm.plantPrice} onChange={handleEditChange} required />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label htmlFor="ed-type">Type</Label>
                                        <StyledSelect id="ed-type" name="plantType" value={editForm.plantType} onChange={handleEditChange}>
                                ...
                                <FormGroup style={{ marginTop: '0.8rem' }}>
                                    ...
                                    <StyledTextarea id="ed-desc" name="plantDescription" value={editForm.plantDescription} onChange={handleEditChange} placeholder="Describe this plant…" />
                                </FormGroup>
                                ...
                                <Grid2>
                                    <FormGroup>
                                        ...
                                        <StyledInput name="size" value={editForm.size} onChange={handleEditChange} placeholder="e.g. 12-24 inches" />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label>Sunlight</Label>
                                        <StyledInput name="sunlight" value={editForm.sunlight} onChange={handleEditChange} placeholder="e.g. Bright indirect" />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label>Water Needs</Label>
                                        <StyledInput name="water" value={editForm.water} onChange={handleEditChange} placeholder="e.g. When top soil dry" />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label>Soil Type</Label>
                                        <StyledInput name="soil" value={editForm.soil} onChange={handleEditChange} placeholder="e.g. Well draining" />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label>Temperature</Label>
                                        <StyledInput name="temperature" value={editForm.temperature} onChange={handleEditChange} placeholder="e.g. 18-24°C" />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label>Humidity</Label>
                                        <StyledInput name="humidity" value={editForm.humidity} onChange={handleEditChange} placeholder="e.g. 40-60%" />
                                    </FormGroup>
                                </Grid2>
                                ...
                                <FormGroup>
                                    ...
                                    <StyledInput name="fertilizer" value={editForm.fertilizer} onChange={handleEditChange} placeholder="e.g. Every 2-4 weeks in summer" />
                                </FormGroup>
                                <FormGroup style={{ marginTop: '0.8rem' }}>
                                    <Label>Primary Nutrients (comma separated)</Label>
                                    <StyledInput name="nutrients" value={editForm.nutrients} onChange={handleEditChange} placeholder="e.g. Nitrogen, Phosphorus" />
                                </FormGroup>
                                ...
                                <Grid2>
                                    <FormGroup>
                                        <Label>Origin</Label>
                                        <StyledInput name="origin" value={editForm.origin} onChange={handleEditChange} placeholder="e.g. Tropical regions" />
                                    </FormGroup>
                                    <FormGroup>
                                        <Label>Pet Safe?</Label>
                                        <StyledSelect name="petSafe" value={editForm.petSafe} onChange={handleEditChange}>
                                ...
                                <FormGroup style={{ marginTop: '0.8rem' }}>
                                    ...
                                    <StyledInput name="commonIssues" value={editForm.commonIssues} onChange={handleEditChange} placeholder="e.g. Yellow leaves = overwatering" />
                                </FormGroup>
                                ...
                                <FormGroup>
                                    ...
                                    <StyledInput type="file" accept="image/*" onChange={handleImageEdit}
                                        style={{ padding: '0.6rem', background: '#f5f5f5', border: '2px dashed #ccc', cursor: 'pointer' }}
                                    />
                                ...
                                <ModalActions>
                                    ...
                                    <CancelBtn type="button" onClick={() => setEditingPlant(null)}>Cancel</CancelBtn>
                                </ModalActions>
                            </form>
                        </Modal>
                    </Overlay>
                ...
                <StyledSection>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
                        ...
                        <Link to="/nursery/add-plant" style={{ textDecoration: 'none' }}>
                            <button style={{
                                padding: '1rem 2rem', background: '#304c40', color: 'white',
                    ...
                    <StyledTable>
                        ...
                        <tbody>
                            {[...myPlants].reverse().slice(0, 5).map(plant => (
                                <tr key={plant.id || plant.plantSlug}>
                                    <StyledTd><strong>{plant.plantTitle}</strong></StyledTd>
                                    <StyledTd>${plant.plantPrice}</StyledTd>
                                    <StyledTd>{Math.floor(Math.random() * 100) + 10}</StyledTd>
                                    <StyledTd><span style={{ color: 'green' }}>In Stock</span></StyledTd>
                                    <StyledTd>
                                        <EditBtn onClick={() => openEdit(plant)}
                ...
                {myNursery && myNursery.status === 'pending' && (
                    <div style={{
                        padding: '2rem', background: '#fff3cd',
                        border: '1px solid #ffeeba', borderRadius: '10px', color: '#856404'
                    }}>
                        <h3>⚠️ Account Pending Approval</h3>
                        <p style={{ marginTop: '1rem' }}>
                            Your nursery account is under review. Plants won&apos;t appear in the public shop until approved.
                        </p>
                    </div>
                )}
            </StyledDashboard>
