...
const NurseryProvider = ({ children }) => {
    ...
    const editNursery = (updatedNursery) => {
        setNurseries(
            nurseries.map((n) => (n.id === updatedNursery.id ? updatedNursery : n))
        );
    };

    const approveNursery = async (id) => {
        try {
            const res = await fetch(`http://localhost:5000/api/nurseries/${id}/approve`, { method: 'PUT' });
            if (res.ok) {
    ...
    const rejectNursery = async (id) => {
        try {
            const res = await fetch(`http://localhost:5000/api/nurseries/${id}/reject`, { method: 'PUT' });
            if (res.ok) {
