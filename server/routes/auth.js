...
router.post('/signup', async (req, res) => {
    try {
        ...
        res.json({ token, user: { uid: savedUser.id, email: savedUser.email, displayName: savedUser.fullName, phoneNumber: savedUser.phoneNumber, role: 'user' } });
    } catch (err) {
...
router.post('/login', async (req, res) => {
    try {
        ...
        res.json({ token, user: { uid: user.id, email: user.email, displayName: user.fullName, phoneNumber: user.phoneNumber, role: 'user' } });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
});
...
router.post('/request-reset', async (req, res) => {
    try {
        ...
        let collection = 'user';
        if (!user) { user = await Admin.findOne({ where: { email } }); collection = 'admin'; }
        if (!user) { user = await Nursery.findOne({ where: { email } }); collection = 'nursery'; }
...
router.post('/nursery/login', async (req, res) => {
    try {
        ...
        res.json({ token, user: { uid: nursery.id, email: nursery.email, displayName: nursery.storeName, role: 'nursery' } });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
});
...
router.post('/nursery/signup', async (req, res) => {
    try {
        ...
        if (existingNursery) return res.status(400).json({ message: 'Email already exists! Please use a different email or log in.' });

        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);
        const savedNursery = await Nursery.create({ email, password: hashedPassword, storeName, licenseNumber, phoneNumber, location, status: 'pending' });
        const token = jwt.sign({ id: savedNursery.id, role: 'nursery' }, process.env.JWT_SECRET);
        res.json({ token, user: { uid: savedNursery.id, email: savedNursery.email, displayName: savedNursery.storeName, role: 'nursery' } });
    } catch (err) {
        console.error("Nursery Registration Error:", err);
        res.status(500).json({ message: err.errors ? err.errors.map(e => e.message).join(', ') : err.message });
    }
});
...
router.post('/feedback', async (req, res) => {
    try {
        ...
        const feedback = await Feedback.create({ name, email, rating: Number(rating), category: category || 'General', message });
